

export const ORGANISATION_CREATED_SUCCESS: string = 'Your profile has been created successfully';
export const ORGANISATION_CREATED_FAILED: string = 'Internal server triggered an error';
export const ORGANISATION_ALREADY_EXIST: string = 'This profile is already in our system';
export const ORGANISATION_NOT_EXIST: string = 'This profile is not exist in our system';

export const ORGANISATION_UPDATED_SUCCESS: string = 'Your organisation profile has been updated successfully';
export const ORGANISATION_UPDATED_FAILED: string = 'Your organisation profile not updated due to an internal error';

export const PROJECT_CREATED_SUCCESS: string = 'Your project has been created successfully';
export const PROJECT_CREATED_FAILED: string = 'Project not saved due to an internal error';

export const PROJECT_LIST_SUCCESS: string = 'Your project list has been retrieved successfully';
export const PROJECT_LIST_FAILED: string = 'Projects not retrieved due to an internal error';

export const PROJECT_DETAIL_SUCCESS: string = 'Your project detail has been retrieved successfully';
export const PROJECT_DETAIL_FAILED: string = 'Project detail not retrieved due to an internal error';

export const VERIFICATION_CODE_RESENT_SUCCESS: string = 'Verification code has been sent successfully';
export const VERIFICATION_CODE_RESENT_FAIL: string = 'Verification not sent due to internal server error';

export const PROFILE_APPROVED_SUCCESS: string = 'Profile has been approved successfully';
export const PROFILE_APPROVED_FAIL: string = 'Profile not approved due to an internal error';

export const VERIFICATION_CODE_CONFIRM_SUCCESS: string = 'Verification code has been confirmed successfully';
export const INVALID_VERIFICATION_CODE: string = 'You entered invalid verification code';
export const VERIFICATION_CODE_CONFIRM_FAIL: string = 'Verification code not confirmed due to internal error';

export const WEB_SOCKET_USER_CONNECTION_SUCCESS: string = 'Connect set for user ';
export const WEB_SOCKET_DISCONNECT_SUCCESS: string = 'Web Socket dis-connected for organisation ';

export const PROJECT_PIC_UPLOADED_SUCCESS: string = 'Project timeline pictures has been uploaded successfully';
export const PROJECT_PIC_UPLOADED_FAIL: string = 'Project timeline pictures not uploaded due to an internal error';

export const FILE_ACCEPT_IMAGE: string = 'Only image related file can be acceptable';

export const REQUEST_SUBMIT_SUCCESS: string = 'Your request has been submitted successfully';
export const REQUEST_SUBMIT_FAILED: string = 'Your request not submitted due to an internal error';

export const ORG_PROFILE_SUCCESS: string = 'Your profile has been retrieved successfully';
export const ORG_PROFILE_FAILED: string = 'Your profile not retrieved due to an internal error';

